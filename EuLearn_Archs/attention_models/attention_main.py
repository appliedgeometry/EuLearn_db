...
parser.add_argument("--save", help="File to save model", default='attention.model')
...
parser.add_argument("--load_model", help="Name of previous trained model", default='attention.model')
parser.add_argument("--num_clases", help="Total number of expected classes", default=11)
args = parser.parse_args()

device = 'cuda' if torch.cuda.is_available() else 'cpu'

model = HeadAttention(input_size=3, output_size=int(args.num_clases), d_model=int(args.d_model), dropout=float(args.dropout)).to(device)
try:
