...
class PointNetSetAbstraction(nn.Module):
    def __init__(self, in_channel=3, mlp=[64,128], group_all=False, npoint=256, radius=10, nsample=64):
        ...
        self.npoint = npoint # Number of grouped points
        self.radius = radius # Sampling radius
        self.nsample = nsample # Sample size
        self.mlp_convs = nn.ModuleList()
        self.mlp_bns = nn.ModuleList()
        self.in_channel = in_channel
        last_channel = in_channel # Output channels
        for out_channel in mlp: # Creates convolutional layers
            self.mlp_convs.append(nn.Conv2d(last_channel, out_channel, 1)) # Convolutional layer
            self.mlp_bns.append(nn.BatchNorm2d(out_channel)) # Bacth normalization
            last_channel = out_channel
        self.group_all = group_all

    def forward(self, xyz, points):
        # Checks if it's final or intermediate grouping
        if self.group_all:
        ...
        # Applies convolutions and normalization
        new_points = new_points.permute(0, 3, 2, 1)
        for i, conv in enumerate(self.mlp_convs):
            bn = self.mlp_bns[i]
            new_points =  F.relu(bn(conv(new_points)))

        # Returns permutation and obtains max aggregation
        new_points = new_points.permute(0, 3, 2, 1)
