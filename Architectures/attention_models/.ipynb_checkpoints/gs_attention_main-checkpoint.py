...
from gs_attention import GraphAttention
...
parser.add_argument("--test_file", help="Name of eval files",default='results_att.txt')
parser.add_argument("--epochs", help="Size of sample of points",default=100)
parser.add_argument("--save", help="File to save model", default='gs_attention.model')
...
parser.add_argument("--load_model", help="Name of previous trained model", default='attention_.model')
parser.add_argument("--num_clases", help="Total number of expected classes", default=11)
args = parser.parse_args()

device = 'cuda' if torch.cuda.is_available() else 'cpu'

model = GraphAttention(input_size=3, output_size=int(args.num_clases), d_model=int(args.d_model), dropout=float(args.dropout)).to(device)
try:
